...
class Summarization(App):
    ...
    def __init__(self, X, y, X_test, y_test, model_family='resnet18', model_checkpoint_dir="./checkpoints"):
    ...
    def run(self, measure,  x_ratio=0.2, batch_size=128, epochs=15, HtoL=False):
        ...
        for j in range(0, count, interval):
            if len(keep_idxs) == len(self.X):
                x_train_keep, y_train_keep = self.X, self.y
            else:
                x_train_keep, y_train_keep = self.X[keep_idxs], self.y[keep_idxs]
            ...
            acc, loss = plot_tiny_train("val", knn_resnet, device, self.X_test, self.y_test, optimizer, criterion, scheduler, batch_size)
