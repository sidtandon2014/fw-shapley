...
class LabelPlotter(Plotter):
    ...
    def __init__(self, app, *argv):
        ...
        self.app = app
        self.argv = argv
        self.colormap = {'TMC-Shapley': 'blue', 'G-Shapley': 'orange', 'Leave-One-Out': 'olive'
                        , 'KNN-LOO': 'violet', 'KNN-Shapley': 'purple','FW-Shapley':'green'}
        self.colors = Queue()
    ...
    def plot(self):
        ...
        for (name, result) in self.argv:
            ...
            for i in range(data_num):
                ...
                try:
                    ...
                    f.append(0)
            print('name:', name, ' ,cnt: ',cnt,' ,total: ', total)
            x = np.array(range(1, data_num + 1)) / data_num * 100
            x = np.append(x[0:-1:100], x[-1])
            f = np.append(f[0:-1:100], f[-1])
            plt.plot(x, np.array(f) * 100, '--', color = self.getColor(name), linewidth=2, label = name)
        ...
        x = np.array(range(1, data_num + 1)) / data_num * 100
        f = x / 100
        plt.plot(x, np.array(f) * 100, '--', color='red', linewidth=2, label = "Random", zorder=7)
